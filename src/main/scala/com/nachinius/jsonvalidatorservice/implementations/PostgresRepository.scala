...
object PostgresRepository {
  ...
  object Metas {
    implicit val schemaIdMeta: Meta[SchemaId] =
      Meta[String].timap[SchemaId]((x: String) => SchemaId(x))((schemaId: SchemaId) => schemaId.value)
    ...
    implicit val jsonSchemaRead: Read[JsonSchema] = Read[(SchemaId, JsonDocument)].map { case (id, doc) =>
      JsonSchema(id, doc)
    }
  }
}
